<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">Section blocks for hero, cards, carousels and content areas.</p>
        ...
        <div class="mt-4 overflow-x-auto">
            <table class="admin-items-table min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-3 py-8 text-center text-sm text-slate-500">No content blocks yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($previewBlock)
        @php
            ...
            $safeBodyHtml = strip_tags((string) ($previewTranslation?->body_html ?? ''), '<p><br><strong><em><ul><ol><li><a><h2><h3><h4><blockquote>');
        @endphp
                ...
                <div class="grid flex-1 gap-4 overflow-y-auto p-5 lg:grid-cols-[18rem_1fr]">
                    ...
                    <div class="space-y-4">
                        <div class="rounded-xl border border-slate-200 bg-white p-4">
                            ...
                            @endif
                            @if (!empty($safeBodyHtml))
                                <div class="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">{!! $safeBodyHtml !!}</div>
                            @endif
                        </div>

                        <div class="grid gap-4 lg:grid-cols-2">
                            <div class="rounded-xl border border-slate-200 bg-white p-4">
                                <p class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Block Payload JSON</p>
                                <pre class="mt-2 max-h-52 overflow-auto rounded-lg bg-slate-50 p-3 text-xs text-slate-700">{{ json_encode($previewBlock->payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) ?: '{}' }}</pre>
                            </div>
                            <div class="rounded-xl border border-slate-200 bg-white p-4">
                                <p class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Translation Payload JSON</p>
                                <pre class="mt-2 max-h-52 overflow-auto rounded-lg bg-slate-50 p-3 text-xs text-slate-700">{{ json_encode($previewTranslation?->payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) ?: '{}' }}</pre>
                            </div>
                        </div>
                    </div>
                </div>

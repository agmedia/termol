<div class="space-y-6">
    <div class="admin-panel admin-form-panel p-6">
        <p class="admin-section-title">Manual Adjustment</p>
        <div class="mt-4 grid gap-3" style="grid-template-columns: minmax(18rem, 1.2fr) minmax(18rem, 1.2fr) 8rem minmax(20rem, 1.6fr) 8rem;">
            ...
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Order Search (Optional)</label>
                <input
                    type="text"
            ...
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Points +/-</label>
                <input
                    type="number"
            ...
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Reason</label>
                <input
                    type="text"
    ...
    <div class="admin-panel admin-search-panel p-6">
        <div>
            <h1 class="text-xl font-semibold tracking-tight">User Loyalty</h1>
            <p class="mt-1 text-sm text-slate-600">Ledger of loyalty points with user, type, date, and points filters.</p>
            ...
            @if ($selectedUser)
                <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-600">
                    <span class="admin-chip">Scoped User: {{ $selectedUser->name }}</span>
                    <span class="admin-chip">Balance: {{ $selectedUserBalance }} pts</span>
        ...
        <div class="mt-4 grid items-end gap-3" style="grid-template-columns: minmax(24rem, 1.4fr) 8rem 12rem 10rem 10rem 8rem 8rem;">
            <div>
                ...
                <input
                    ...
                    placeholder="Name, email, event key..."
                    class="admin-search-input w-full rounded-xl border px-3 py-2 text-sm"
                />
            </div>
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">User ID</label>
                <input
                    type="number"
                    min="1"
            ...
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Date From</label>
                <input type="date" wire:model.live="dateFrom" class="admin-search-input w-full rounded-xl border px-3 py-2 text-sm" />
            </div>
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Date To</label>
                <input type="date" wire:model.live="dateTo" class="admin-search-input w-full rounded-xl border px-3 py-2 text-sm" />
            </div>
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Points Min</label>
            ...
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Points Max</label>
        ...
        <div class="mt-4 flex flex-wrap items-center gap-2">
            <span class="admin-chip">Rows: {{ $stats['rows'] }}</span>
            <span class="admin-chip">Points Sum: {{ $stats['points_sum'] }}</span>
            <span class="admin-chip">Users: {{ $stats['users_count'] }}</span>
        </div>
    </div>
        ...
        <div class="mt-4 overflow-x-auto">
            <table class="admin-items-table min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($rows as $row)
                        <tr>
                            ...
                            <td class="px-3 py-2 text-slate-800">
                                ...
                                @else
                                    <span class="text-xs text-slate-500">Deleted user</span>
                                @endif
                            </td>
                            ...
                            <td class="px-3 py-2 text-slate-700">
                                @if ($row->order)
                                    ...
                                    <div class="text-xs text-slate-500">{{ number_format((float) $row->order->grand_total, 2) }} {{ $row->order->currency_code }}</div>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-3 py-8 text-center text-sm text-slate-500">No loyalty transactions found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

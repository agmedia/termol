<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{{ __('Catalog / Manufacturers') }}</p>
                <h1 class="mt-2 text-2xl font-semibold tracking-tight text-slate-900">{{ $isEdit ? __('Edit Manufacturer') : __('Create Manufacturer') }}</h1>
                <p class="mt-2 text-sm text-slate-600">{{ __('Core manufacturer data and locale translation content.') }}</p>
            </div>
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="admin-panel admin-form-panel p-6">
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3">
                <button
                    ...
                    aria-label="{{ __('Toggle manufacturer active state') }}"
                >
                ...
                <button
                    ...
                    aria-label="{{ __('Toggle manufacturer featured state') }}"
                >
                    <span class="admin-switch-track"><span class="admin-switch-thumb"></span></span>
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <div class="admin-panel admin-form-panel p-6">
                ...
                <div class="mt-3">
                    <label for="manufacturer-description-html" class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('Description') }}</label>
                    <textarea id="manufacturer-description-html" rows="6" wire:model.live.debounce.300ms="form.description" data-quill-editor class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"></textarea>
                </div>
            ...
            <div class="admin-panel admin-form-panel p-6">
                ...
                <div class="mt-4">
                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('Manufacturer Payload JSON') }}</label>
        ...
        <div class="admin-form-actions flex items-center gap-2 pt-2">
            <button type="submit" class="rounded-xl bg-cyan-700 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-800">
                {{ $isEdit ? __('Update Manufacturer') : __('Create Manufacturer') }}
            </button>

<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        ...
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-xl font-semibold tracking-tight">Option Values</h1>
                <p class="mt-1 text-sm text-slate-600">Option: <span class="font-semibold text-slate-800">{{ $optionName }}</span> ({{ $option->code }})</p>
                        ...
                        <tr>
                            ...
                            <td class="px-3 py-2 text-right">
                                <div class="inline-flex items-center gap-1">
                                    ...
                                    <button type="button" wire:click="delete({{ $row->id }})" wire:confirm="Delete this value?" class="rounded-lg border border-rose-200 px-2 py-1 text-xs font-semibold text-rose-700 hover:bg-rose-50">Delete</button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-3 py-8 text-center text-sm text-slate-500">No values yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="admin-panel admin-form-panel p-6">
        <h2 class="admin-section-title">{{ $editingId ? 'Edit value' : 'Create value' }}</h2>
        ...
        <form wire:submit="save" class="admin-form mt-4 space-y-4">
            <div class="grid gap-3" style="grid-template-columns: repeat(12, minmax(0, 1fr));">
                ...
                <div style="grid-column: span 4;" class="flex items-end">
                    <button
                        ...
                        aria-label="Toggle option value active state"
                    >
            ...
            <div class="grid gap-3 md:grid-cols-2">
                <div>
                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Value Payload JSON</label>

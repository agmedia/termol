    ...
    <div class="admin-panel admin-search-panel p-6">
        <div class="flex items-end justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">Snapshot-based order flow with item, totals, status and timeline history.</p>
            ...
            <div class="flex w-[74rem] max-w-full items-end justify-end gap-3">
                <div class="grid w-full max-w-[72rem] items-end gap-3" style="grid-template-columns: minmax(30rem, 1fr) 13rem 10rem 10rem;">
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Placed From</label>
                        <input type="date" wire:model.live="dateFrom" class="admin-search-input w-full rounded-xl border px-3 py-2 text-sm" />
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Placed To</label>
        ...
        <div class="mt-4 overflow-x-auto">
            <table class="admin-items-table min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($rows as $row)
                        ...
                        <tr>
                            ...
                            <td class="px-3 py-2 text-slate-800">
                                ...
                                <div class="text-xs text-slate-500">{{ $row->items_count }} items</div>
                            </td>
                            ...
                            <td class="px-3 py-2 text-center text-slate-700">
                                <div class="font-semibold">{{ number_format((float) $row->grand_total, 2) }} {{ $row->currency_code }}</div>
                                <div class="text-xs text-slate-500">{{ number_format((float) $row->subtotal, 2) }} subtotal</div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-3 py-8 text-center text-sm text-slate-500">No orders yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

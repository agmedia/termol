<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        <h1 class="text-xl font-semibold tracking-tight">Admin Appearance Controls</h1>
        <p class="mt-2 text-sm text-slate-600">
            Settings namespace: <code>Settings/System/AdminAppearanceControls</code>
        </p>
    </div>

    <div class="grid gap-6 lg:grid-cols-2">
        <div class="admin-panel admin-panel-soft p-6">
            <p class="admin-section-title">Admin Pagination</p>
            <p class="mt-3 text-sm text-slate-600">Used by admin item tables and category roots tree.</p>

            <form wire:submit="save" class="admin-form mt-4 space-y-4">
                <div>
                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Admin Items Per Page</label>
                ...
                <div>
                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Admin Category Roots Per Page</label>
        ...
        <div class="admin-panel admin-panel-soft p-6">
            <p class="admin-section-title">Frontend Pagination</p>
            <p class="mt-3 text-sm text-slate-600">Prepared now so front Vue templates can consume same values from one source.</p>

            <form wire:submit="save" class="admin-form mt-4 space-y-4">
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Category Products (Desktop)</label>
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Category Products (Mobile)</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Manufacturer Products (Desktop)</label>
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Manufacturer Products (Mobile)</label>
                ...
                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600">
                    Recommendation: keep desktop 20-30 items and mobile 8-16 items for better UX + cache hit consistency.
                </div>
            </form>
        </div>
    </div>

<div class="space-y-6">
    <div class="admin-panel admin-search-panel p-6">
        <h1 class="text-xl font-semibold tracking-tight">User Settings</h1>
        <p class="mt-2 text-sm text-slate-600">Namespace: <code>Settings/User</code></p>
        <p class="mt-2 text-xs text-slate-500">Control user tracking capture and loyalty engine behavior.</p>
    </div>

    <div class="admin-panel admin-form-panel p-6">
        <form wire:submit="save" class="admin-form mt-1 space-y-4">
            <div>
                <p class="admin-section-title">Feature Switches</p>
                <div class="mt-3 grid gap-3 md:grid-cols-2">
                    @php
                        $switches = [
                            'user_tracking_enabled' => [
                                'title' => 'User Tracking',
                                'description' => 'Stores user/front interaction events for analytics and personalization.',
                            ],
                            'user_loyalty_enabled' => [
                                'title' => 'Loyalty System',
                                'description' => 'Awards loyalty points from order totals when eligible statuses are reached.',
                            ],
                        ];
                    @endphp
            ...
            <div>
                <p class="admin-section-title">Loyalty Rules</p>
                <div class="mt-3 grid gap-3 md:grid-cols-3">
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Points Per Currency Unit</label>
                        <input
                            type="number"
                            step="0.01"
                            min="0"
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Minimum Order Total</label>
                        <input
                            type="number"
                            step="0.01"
                            min="0"
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Reversal Policy</label>
                        <select wire:model="form.loyalty_reversal_mode" data-tom-select data-tom-no-search="1" class="admin-select w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
                            <option value="zero_out">Zero out settlement row</option>
                            <option value="separate_entry">Create separate reversal entry</option>
                        </select>
                ...
                <div class="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                    Award formula: <code>round(order grand total * points per currency)</code>, only when order status is marked paid and not cancelled. Reversal policy controls what happens when an awarded order moves to cancelled/refunded flow.
                </div>
            </div>

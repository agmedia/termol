@php
    ...
    $priceData = app(\App\Services\Pricing\ProductPricePresentationService::class)->forProduct($product, auth()->user());
    $displayPrice = (float) ($priceData['current_gross'] ?? 0);
    ...
    <div class="content">
        <div class="d-flex">
            <div class="w-100 pe-2">
                ...
                @endif
                @if (!empty($priceData['is_b2b_price']))
            ...
            >
                ...
                <button type="submit" class="btn btn-3d btn-xs font-600 bg-highlight d-inline-flex align-items-center justify-content-center gap-1">
                    <x-fa-icon name="bag-shopping" class="me-1 h-4 w-4" />
                    Add
                </button>
            </form>
        </div>

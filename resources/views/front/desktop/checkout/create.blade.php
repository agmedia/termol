                ...
                <table class="min-w-[520px] w-full text-sm">
                    ...
                    @foreach ($lines as $line)
                        ...
                        <tr class="border-t border-slate-200">
                            ...
                            <td class="px-3 py-2 align-top font-semibold">EUR {{ number_format((float) ($line['display_line_total'] ?? $line['line_total']), 2) }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
            ...
            <div class="mt-4 border-t border-slate-200 pt-4 text-sm">
                ...
                @if ((float) ($summary['discount_total'] ?? 0) > 0)
                    <div class="mt-2 flex items-center justify-between">
                        ...
                        <span class="font-semibold text-emerald-700">-EUR {{ number_format((float) $summary['discount_total'], 2) }}</span>
                    </div>
                @endif
                <div class="mt-2 flex items-center justify-between">
                    <span class="text-slate-600">Tax</span>
                    <span class="font-semibold text-slate-900">EUR {{ number_format((float) ($summary['tax_total'] ?? 0), 2) }}</span>
                </div>
                <div class="mt-2 flex items-center justify-between border-t border-slate-200 pt-2">
                    <span class="text-slate-900">Total</span>
                    <span class="font-bold text-slate-900">EUR {{ number_format((float) ($summary['grand_total'] ?? 0), 2) }}</span>
                </div>
            </div>

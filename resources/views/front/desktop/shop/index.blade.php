...
@section('content')
    ...
    <section class="relative z-20 mx-3 mt-3 border border-slate-200 bg-white p-4 sm:mx-4 lg:mx-6">
            ...
            <form method="GET" action="{{ route('shop.index') }}" class="mt-3 hidden gap-3" data-mobile-filter-panel id="shop-mobile-filter-panel">
                ...
                @if ($hasActiveFilters)
                    <div class="flex items-end justify-end">
                        <a href="{{ route('shop.index') }}" class="inline-flex h-[42px] items-center justify-center gap-2 border border-rose-600 px-4 text-sm font-semibold text-rose-600 hover:bg-rose-50">
        ...
        <form method="GET" action="{{ route('shop.index') }}" class="hidden gap-3 min-[1025px]:grid {{ $showManufacturers ? 'min-[1025px]:grid-cols-[1fr_1fr_1fr_1fr_auto_auto]' : 'min-[1025px]:grid-cols-[1fr_1fr_1fr_auto_auto]' }}" data-desktop-filter-form>
            <input type="hidden" name="q" value="{{ $filters['q'] }}">
            <div>

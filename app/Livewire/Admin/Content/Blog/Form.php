...
class Form extends Component
{
    ...
    public string $lastAutoGeneratedCode = '';
    ...
    public function updatedFormTitle($value): void
    {
        ...
        $slug = Str::slug($title);
        if ($slug !== '') {
            $currentSlug = trim((string) ($this->form['slug'] ?? ''));
            if ($currentSlug === '' || $currentSlug === $this->lastAutoGeneratedSlug) {
                $this->form['slug'] = $slug;
                $this->lastAutoGeneratedSlug = $slug;
            }

            $currentCode = trim((string) ($this->form['code'] ?? ''));
            if ($currentCode === '' || $currentCode === $this->lastAutoGeneratedCode) {
                $generatedCode = $this->uniqueCodeFromBase($slug);
                $this->form['code'] = $generatedCode;
                $this->lastAutoGeneratedCode = $generatedCode;
            }
        }

...
class ImportedDescriptionHtmlCleaner
{
    public function clean(string $value): string
    {
        ...
        try {
            ...
            $this->removeElementsByTagName($root, 'style');
            $this->removeElementsByTagName($root, 'script');
            $this->removeStyleAttributes($root);
    ...
    {
        $cleaned = preg_replace('#<(style|script)\b[^>]*>.*?</\1>#is', '', $html) ?? $html;
    ...
    private function removeStyleAttributes(DOMNode $node): void
    {
        ...
        foreach ($children as $childNode) {
            $this->removeStyleAttributes($childNode);
        }
    }
